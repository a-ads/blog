...
export const Head: HeadFC = () => <Seo title='Blog Post Page' />

const TableOfContents = ({
  toc,
  className,
}: {
  toc: BlogPostPageContext['table_of_contents']
  className?: string
}) => (
  // Todo clean up this messy component
  <nav className={cn('relative toc-gatsby-config', className)}>
    <div className='toc up-desktop:sticky top-0 left-0 down-desktop:mt-8 down-desktop:mb-10 phone:my-7'>
...
const BlogPostTemplate: React.FC<{ pageContext: BlogPostPageContext }> = ({
  pageContext,
}) => {
  const { post, author, html, table_of_contents, related_posts } = pageContext
